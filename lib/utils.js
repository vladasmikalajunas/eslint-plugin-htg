...
const createUtils = (cwd, settings) => {
    ...
    const getWildcard = (path, realPath) => {
        ...
        return {
            prefix: parts[0],
            value: realPath.replace(parts[0], '').split('/')[0],
        };
    }
    ...
    const createFileObject = (raw, realPath, path) => {
        ...
        if (wildcard) {
            response.wildcard = wildcard.value;
            response.wildcardPrefix = wildcard.prefix;
        }
    ...
    return {
        ...
        dumpEnv: () => ({
            settingsPath, settingsModules, cwd, categories, aliases, settings
        })
    };
}
